...
def get_seed_modules(network,seed_nodes,random_walk_weights_pvals,pval_thresh,weight_thresh,max_subnet_size,vertex_index,output_file,output_dir):
	...
	seed_subnet_comp = [cc for cc in connected_component_subgraphs(seed_subnet_with_added_nodes) if cc.number_of_nodes()>1]
    #seed_subnet_comp = [cc for cc in list(nx.connected_component(seed_subnet_with_added_nodes)) if len(cc)>1] ## TO DO
	# plot the subnetwork
